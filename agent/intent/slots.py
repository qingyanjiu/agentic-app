...
def extract_person_status_slots(query: str) -> dict:
    ...
    # ============================================================
        ...
        r"进入多少|进来多少|入园多少|来.*多少|进.*多少|入.*多少|"
        r"(?:那|再|就)?(?:进入|进来|入园|来了?)[的呢了吧得呀哦嘛]{0,3}$", q
    ):
    ...
    # ============================================================
        ...
        r"出去几个|走了几个|离开几个|"
        r"离开多少|出去多少|走了多少|出.*多少|走.*多少|离.*多少|"  # 离.*多少 补「离开园区多少人」句式（原漏写，被 realtime 的 园区.*多少.*人 兜底接走）
        r"(?:那|再|就)?(?:出去|离开|离园|走了?)[的呢了吧得呀哦嘛]{0,3}$", q
    ):
