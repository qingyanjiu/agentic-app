...
说明：
    ...
    - 为增加口气多样性，脚本按 5 种风格分别生成：一般疑问、省略主语疑问、口语化疑问、场景化疑问、陈述式请求。
    - 统一硬性要求：4~20 字、口语化、带子类型核心词锚定；疑问/陈述均可（按风格），时间口径必须与参考表达一致。
"""
...
STYLES = [
    ...
    {
        "name": "陈述式请求",
        "desc": "陈述/请求口吻，不带疑问词，像用户直接下指令，比如'帮我看下XX'、'调出XX的数据'、'我要看XX情况'、'给我列一下XX'、'XX数据发我'",
    },
]
...
约束：
- 每条可以是疑问句或陈述/请求式：疑问句带疑问词（吗/呢/多少/有没有/在哪/怎么/哪些/什么等）；陈述式不带疑问词（如"帮我看下XX""调出XX数据"），禁止书面腔。
- 每条 4~20 个字，像真实用户在输入框里敲出来的短句。
- 必须口语化，是真实用户在“{sub_type}（{sub_type_desc}）”场景下自然会问出的话，不要书面语、不要机器味、避免礼貌用语（不要“请问”“您好”“谢谢”）。
- 口气要符合“{style_name}”风格：疑问类风格不要输出陈述句，陈述式风格不要硬加疑问词。
- 每条表达必须和子类型含义强相关，不要跑题。
- 每条都要带上该子类型的核心词（从子类型说明和参考表达里提取，如"能耗/电表/报修/巡检/防区/告警"这类词），禁止写成与参考表达脱节的泛泛问法。
- 时间口径必须与参考表达一致：参考表达里没有的时间词不要引入；参考表达不含"实时"时禁止使用"实时"一词（平台数据无实时口径）。
- 不要重复或近似表达，必须覆盖不同角度。
