...
from agent.intent.slots import extract_twins_inspection_slots, _parse_chinese_number, parse_ordinal_choice, parse_point_day
from agent.intent.classifier import classify_inspection_sub_type
...
# ============================================================
class TwinsInspectionHandler(IntentHandler):
    name = "twins_inspection"
    ...
    async def extract_slots(self, query: str, is_followup: bool = False) -> dict:
        ...
        """
        # 1. 正则抽取所有字段
        slots = extract_twins_inspection_slots(query)

        # 2. 只有正则落到 count 兜底时才交给分类器补判（追问轮不重判子类型）
        if slots.get("query_type") == "count" and not is_followup:
            try:
                sub_type, score = await classify_inspection_sub_type(query)
                logger.info(
                ...
                logger.warning(f"巡检子类型分类失败，使用正则兜底结果: {e}")
    ...
    def is_related(self, state: IntentState, query: str) -> bool:
        ...
        # 4. 如果用户回复了孪生巡检相关关键词，也判定为相关
        inspection_keywords = [
            "巡检", "巡查", "任务", "点位", "完成率", "班组",
            "隐患", "执行", "统计", "正常", "异常"
        ]
        if any(k in query for k in inspection_keywords):
            return True
    ...
    async def handle_reply(self, state: IntentState, query: str, llm) -> dict:
        ...
        if state.slots.get("_pending_type_clarify"):
            options = state.slots.get("_type_options") or [
                "today_inspection", "today_tasks",
                "inspection_statistics", "inspection_execution_status"
            ]
    ...
    def _resolve_type_choice(self, query: str, options: list):
        ...
        t = extract_twins_inspection_slots(query)["query_type"]
        if t in options:
            return t
