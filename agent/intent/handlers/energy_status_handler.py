...
from agent.intent.slots import extract_energy_status_slots, _parse_chinese_number, parse_point_day
from agent.intent.classifier import classify_energy_sub_type
...
# ============================================================
class EnergyStatusHandler(IntentHandler):
    name = "energy_status"
    ...
    async def extract_slots(self, query: str) -> dict:
        ...
        """
        # 1. 正则抽取所有字段
        slots = extract_energy_status_slots(query)

        # 2. 只有正则落到 count 兜底时才交给分类器补判
        if slots.get("query_type") == "count":
            try:
                sub_type, score = await classify_energy_sub_type(query)
                logger.info(
                ...
                logger.warning(f"能源子类型分类失败，使用正则兜底结果: {e}")
    ...
    def is_related(self, state: IntentState, query: str) -> bool:
        ...
        # 4. 如果用户回复了能源态势相关关键词，也判定为相关
        energy_keywords = [
            "电", "水", "能耗", "能源", "用电", "用水", "耗电", "耗水",
            "电表", "水表", "表具", "排名", "在线", "离线", "趋势", "曲线",
            "统计", "用量", "总量", "累计"
        ]
        if any(k in query for k in energy_keywords):
            return True
