...
from agent.intent.slots import extract_canteen_status_slots, _parse_chinese_number, parse_point_day
...
# 子类型（event_type）：dish_rank（本月菜品热度排行）/ week_menu（本周菜谱）/ dining_count（就餐人数统计）
# MCP 调用传 {startTime, endTime}（可选带 meal 餐次）
# ============================================================
class CanteenStatusHandler(IntentHandler):
    ...
    def is_related(self, state: IntentState, query: str) -> bool:
        ...
        # 3. 如果用户回复了食堂相关关键词，也判定为相关
        canteen_keywords = [
            "食堂", "餐厅", "菜", "菜单", "菜谱", "早餐", "午餐", "晚餐",
            "人数", "就餐", "热度", "排行", "热门", "销量", "最受欢迎", "卖得好"
        ]
        if any(k in query for k in canteen_keywords):
            return True
