...
ENERGY_STATUS_EXAMPLES = {
    # 总体能耗（overall_energy）
    # 红线：含「今天/今日/当天/昨日」的用电用水问法（点值/情况/曲线/趋势）都归本组；
    #       本周/上周/周对比归 realtime_electricity / realtime_water
    "overall_energy": [
        "总体能耗",
        "总能耗",
        "能源统计",
        ...
        "用电总量",
        "用水总量",
        # 批1种子：今天+用量形态、情况怎么样泛句式（规则8口径，禁「实时/现在」）
        "今天能耗多少",
        "今天的用电量是多少",
        "今天的用水量是多少",
        "今天用了多少电",
    ...
    "electricity_rank": [
        ...
        "哪个区域用电多",
        "耗电情况排名",
        # 批1种子：保持排名口径，禁裸点值（点值归 overall_energy）
        "哪个单位用电最多",
        "用电量前十的单位",
        "部门用电排行",
        "本月用电排名",
    ...
    # 设备状态（device_status）
    # 红线：必须带「能耗/能源/电表/水表」限定；裸「设备在线率/离线设备」归设备态势域
    "device_status": [
        "能耗设备状态",
        "能源设备状态",
        "能耗设备在线",
    ...
    # 用电周对比（realtime_electricity）
    # 红线：只收本周/上周/对比/趋势曲线口径；今日/今天/昨日问法归 overall_energy；
    #       「实时/现在」类问法不收（工具无实时数据，只有周对比）
...
COMPOSITIVE_OVERVIEW_EXAMPLES = {
    ...
    "basic_info": [
        ...
        "园区设备接入总数",
        "园区设备总数",
        # 批1种子：总览/简介形态（红线：仍禁能耗/会议/安全指数/分类占比类问法）
        "园区简介",
        "园区介绍",
        "综合态势总览",
        "园区基本信息有哪些",
    ...
    "device_health": [
        ...
        "设备健康度如何",
        "设备健康情况怎么样",
        # 批1种子：只收健康总分口径，类别健康仍归 device_status.category_health
        "设备健康分怎么样",
        "园区设备健康分",
        "设备健康指数",
        "设备健康状况总览",
...
EMERGENCY_FIRE_STATUS_EXAMPLES = {
    ...
    "fire_alarm_list": [
        ...
        "现在有什么告警",
        "告警信息",
        # 批1种子：实时告警归本组（规则4，告警列表本就是实时数据，可用「实时」）
        "查看实时告警",
        "当前的消防告警",
        "最新消防告警列表",
        "今天有哪些消防告警",
    ...
    # 月度报修（month_repair）
    # 红线：裸报修（本月报修/报修数量/报修趋势等）归 device_status.month_repair，
    #       本组条目必须带「消防」限定
    "month_repair": [
        "本月消防报修",
        "上月消防报修",
        ...
        "消防维修统计",
        "消防报修",
        # 批1种子：组内只剩 5 条，优先补齐（必须带消防限定，规则2）
        "本月消防报修了多少",
        "消防报修数量",
        "消防报修趋势",
        "这个月消防报修几次",
...
TWINS_INSPECTION_STATUS_EXAMPLES = {
    ...
    "today_inspection": [
        ...
        "孪生巡检概况",
        "巡检整体完成情况",
        # 批1种子：约1/3非今日口径（孪生/总览形态，孪生巡检总览回归修复）
        "孪生巡检数据总览",
        "巡检总览",
        "巡检完成情况怎么样",
        "今日巡检点位完成率",
    ...
    # 一般巡检查询（count）：兜底
    # 红线：本组收页面/入口/兜底口径；「总览/概况/完成率」归 today_inspection
    "count": [
        "打开孪生巡检",
        "数字孪生巡检应用",
        "查下巡检",
