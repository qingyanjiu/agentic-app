...
#### 4.关键事件
按“(1)、”、“(2)、”编号、...连续编号，每项为一段；
(1)、本期重大能耗事件（入侵、误报、异常聚集等）数量及类型: {key_events}（若某类事件数为0，标注“0起”；无相关数据则按规则标注“本期无重大安防事件”）
...
#### 7.改进建议
按“(1)、”、“(2)、”编号、...连续编号，每项为一段；
可落地的能耗优化措施（硬件增补、流程优化、数据更新等方面）:需针对原因剖析、趋势分析、热点区域等结论提出对应措施，每条措施需具体、可落地，如“补建5台全景摄像头”需明确安装位置；至少提出3条，参考示例格式）
例如:
（1）、补建 5 台全景摄像头
（2）、人脸库每周更新
（3）、增设红外围界报警
...
"""
    ...
    #     current_time= time.strftime("%Y-%m-%d %H:%M")  # 补充时间变量（模板中提到但未替换的）
    # )
    prompt = str(prompt)
    return prompt
