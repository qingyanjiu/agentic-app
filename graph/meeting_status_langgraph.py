...
# 与 Java 侧 MeetingMcpServerConfig / PlatformMeetingMcp 对齐：
#   meeting_statistics      -> meeting:getMeetingStatistics
#   high_freq_meeting_rooms -> meeting:getHighFrequencyMeetingRooms
#   meeting_room_overview   -> meeting:getMeetingRoomOverview
#   number_of_meetings      -> meeting:getNumberOfMeetings
#   meeting_room_status     -> meeting:getMeetingRoomStatus
#   room_meet_list_by_day   -> meeting:getRoomMeetListByDay
# ============================================================
_JAVA_TOOL_MAP = {
    "meeting_statistics": "meeting:getMeetingStatistics",
    "high_freq_meeting_rooms": "meeting:getHighFrequencyMeetingRooms",
...
def _query_type_label(event_type: str) -> str:
    ...
    return {
        ...
        "high_freq_meeting_rooms": "高频会议室",
        "meeting_room_overview": "会议室一览",
        "number_of_meetings": "会议数量",
        "meeting_room_status": "会议室状态",
        "room_meet_list_by_day": "当日会议安排",
    }.get(event_type, "会议数据")


async def _llm_format_meeting_result(
    llm: Any, event_type: str, state: dict, raw_text: str
) -> str:
    """
    根据 event_type 让 LLM 分析 MCP 工具返回，生成对应的中文回答。
    ...
    try:
        ...
        requirement = {
            "meeting_statistics": "列出预约数、平均时长及环比涨跌；",
            "high_freq_meeting_rooms": "按使用频率列出会议室TOP5；",
            "meeting_room_overview": "列出各会议室使用情况；",
            "number_of_meetings": "列出会议数量趋势、状态分布和类型分布；",
            "meeting_room_status": "列出会议室使用/空闲占比及明细；",
            "room_meet_list_by_day": "按时间列出当日会议安排（会议名、时间、申请人、状态）；",
        }.get(event_type, "把返回数据整理清楚；")

        prompt = (
            "你是智慧园区会议管理助手。下面是一次 MCP 工具查询的原始返回，"
        ...
        logger.warning(f"LLM 格式化会议管理结果失败，降级为原样返回: {e}")
...
class MeetingStatusGraphState(TypedDict):
    # 从当前意图状态中提取的参数
...
# 图节点函数
# ============================================================
def init_state(state: MeetingStatusGraphState) -> dict:
    """
...
def check_missing_params(state: MeetingStatusGraphState) -> dict:
    """
    检查缺失参数
    会议管理目前不强制需要额外参数
    """
...
def route_missing_params(state: MeetingStatusGraphState) -> str:
    """
    条件路由：根据是否缺失参数决定下一步
    """
...
def ask_param(state: MeetingStatusGraphState) -> dict:
    """
    参数缺失时生成追问问题
    """
...
def give_up(state: MeetingStatusGraphState) -> dict:
    """
    追问次数超限，放弃当前任务
    """
    return {
...
def check_date(state: MeetingStatusGraphState) -> dict:
    """
    日期检查占位节点，实际路由由条件边处理
    """
    return {}
...
def route_date_type(state: MeetingStatusGraphState) -> str:
    """
    根据日期类型路由
    会议管理主要关注今日/实时数据，未来日期直接拒绝
    """
...
def future_date(state: MeetingStatusGraphState) -> dict:
    """
    未来时间直接提醒，不查询
    """
    return {
        ...
        "events": [
            {
                ...
                "data": {
                    "type": "answer",
                    "content": "会议安排目前只能查询今天及历史的，未来的会议还没开始哦。",
                },
            },
            {"event": "custom", "data": {"type": "done"}},
        ],
...
def vague_date(state: MeetingStatusGraphState) -> dict:
    """
    模糊时间先让用户确认具体范围
    """
    date_slots = state["slots"].get("date", {})
    ...
    question = (
        f"您想查询近几天的会议信息？可以直接回复具体天数，例如“近两天”、“近四天”，"
        f"或选择：{options_str}"
    )
...
def make_call_tool_node(tools: dict, llm=None):
    ...
    构造调用 MCP 工具的节点
    """
    async def call_tool(state: MeetingStatusGraphState) -> dict:
        event_type = state["slots"].get("event_type") or "count"
        ...
        # 按 Java 工具签名组装参数
        # getRoomMeetListByDay 为 POST，传 {startTime, endTime}；其余为 GET 无参
        date_slots = state["slots"].get("date", {})
        if event_type == "room_meet_list_by_day":
            tool_args = {
        ...
        try:
            ...
            answer_text = await _llm_format_meeting_result(
...
def finalize(state: MeetingStatusGraphState) -> dict:
    """
    标记流程完成
    """
    return {
...
# 图构建
# ============================================================
def build_meeting_status_graph(tools: dict, llm=None) -> StateGraph:
    """
    构建会议管理 LangGraph 状态图
    ...
    :param llm: 可选的 LLM，用于根据 event_type 分析 MCP 返回生成回答；不传则原样返回
    """
    workflow = StateGraph(MeetingStatusGraphState)
...
# 辅助：加载 MCP 工具
# ============================================================
async def load_meeting_status_tools() -> dict:
    """
    从 MCP 配置加载会议管理工具，并以工具名为 key 返回
    类级缓存由调用方维护
    """
    ...
    try:
        ...
        logger.error("加载 MCP 会议管理工具超时")
        return {}
    except Exception as e:
        logger.error(f"加载 MCP 会议管理工具失败: {e}", exc_info=True)
        return {}

    filtered = [t for t in tools if t.name in java_tool_names]
    if not filtered:
        logger.warning(f"未找到任何会议管理 MCP 工具，期望 {java_tool_names}")
