...
# ============================================================
_JAVA_TOOL_MAP = {
    "parking_space": "vehicle_status:getParkingSpaceStats",
    "traffic_flow": "vehicle_status:getTrafficFlowStats",
    "parking_structure": "vehicle_status:getParkingStructureStats",
    "official_vehicle": "vehicle_status:getOfficialVehicleStats",
    "parking_duration_rank": "vehicle_status:getParkingDurationRank",
    "parking_monitor": "vehicle_status:getParkingMonitorInfo",
    "vehicle_access_record": "vehicle_status:getVehicleAccessRecords",
    "count": "vehicle_status:getTodayVehicleStats",
}
...
def route_confirm(state: VehicleStatusGraphState) -> str:
    ...
    # 车辆态势中大部分统计类查询目前后端仅支持今日数据
    # 通行记录可支持历史查询，故跳过确认
    if query_type in _JAVA_TOOL_MAP and query_type != "vehicle_access_record":
...
def make_call_tool_node(tools: dict, llm=None):
    ...
    async def call_tool(state: VehicleStatusGraphState) -> dict:
        ...
        # 按 Java 工具签名组装参数
        # 目前车辆态势工具多数不需要额外参数；通行记录可按时间范围传参
        tool_args = {}
        date_slots = state["slots"].get("date", {})
        if query_type == "vehicle_access_record":
            if date_slots.get("start_time") and date_slots.get("end_time"):
                tool_args = {
                    "startTime": date_slots["start_time"],
                    "endTime": date_slots["end_time"],
                }
            else:
                tool_args = {"type": "today"}
        elif query_type == "traffic_flow":
            tool_args = {"type": "day"}
