...
# 与 Java 侧 InspectionMcpServerConfig / PlatformInspectionMcp 对齐：
#   today_inspection             -> inspection:getTodayInspection
#   today_tasks                  -> inspection:getTodayTasks
#   inspection_statistics        -> inspection:getInspectionStatistics
#   inspection_execution_status  -> inspection:getInspectionExecutionStatus
# ============================================================
_JAVA_TOOL_MAP = {
    "today_inspection": "inspection:getTodayInspection",
    "today_tasks": "inspection:getTodayTasks",
...
# 巡检子类型反问的固定选项顺序（与 _JAVA_TOOL_MAP / 反问文案顺序一致），
...
def _query_type_label(query_type: str) -> str:
    ...
    return {
        "today_inspection": "今日巡检",
        "today_tasks": "今日任务列表",
        "inspection_statistics": "巡检统计",
        "inspection_execution_status": "巡检执行状态",
    }.get(query_type, "巡检数据")


async def _llm_format_inspection_result(
    ...
    try:
        ...
        requirement = {
            "today_inspection": "汇报今日巡检任务数、巡检点位数、完成率及分时段图表数据；",
            "today_tasks": "列出今日巡检任务（人员/类型/状态/班组/时间）；",
            "inspection_statistics": "汇报平均巡检时长、巡检点位数、隐患数及巡检类型分布；",
            "inspection_execution_status": "按人列出巡检正常/异常次数情况；",
        }.get(query_type, "把返回数据整理清楚；")

        # 省略式追问（如「昨天呢」）时 original_query 仍是上一轮原话，本轮输入里没有
        # 时间词，slots.date 走兜底"今天"，原话里的时间词不代表本次查询；
        # 把真实查询时间显式交给 LLM，避免回答被原话带偏
        ...
        prompt = (
            "你是智慧园区孪生巡检助手。下面是一次 MCP 工具查询的原始返回，"
        ...
        logger.warning(f"LLM 格式化孪生巡检结果失败，降级为原样返回: {e}")
...
class TwinsInspectionGraphState(TypedDict):
    # 从当前意图状态中提取的参数
...
# 图节点函数
# ============================================================
def init_state(state: TwinsInspectionGraphState) -> dict:
    """
...
def check_missing_params(state: TwinsInspectionGraphState) -> dict:
    """
    检查缺失参数
    巡检后端只支持 4 类查询，query_type 落到 count 兜底
...
def route_missing_params(state: TwinsInspectionGraphState) -> str:
    """
    条件路由：根据是否缺失参数决定下一步
    """
...
def ask_param(state: TwinsInspectionGraphState) -> dict:
    """
    参数缺失时生成追问问题
    """
    ...
    if "query_type" in missing:
        ...
        # 用户直接回复选项词即可被识别
        question = "请问您想查询哪类巡检数据？今日巡检、今日任务列表、巡检统计，还是巡检执行状态？"
        # 存下选项清单与等待标记，供追问轮解析「第N个」这类序数指代
        # （仿消防/周界态势的 _pending_type_clarify / _type_options 模式）
        slots = state["slots"]
        slots["_pending_type_clarify"] = True
        slots["_type_options"] = list(_TYPE_OPTIONS)
        updates["slots"] = slots
    elif "date" in missing:
        question = "请问您想查询哪一天的巡检数据？"
    else:
...
def give_up(state: TwinsInspectionGraphState) -> dict:
    """
    追问次数超限，放弃当前任务
    """
    return {
...
def check_date(state: TwinsInspectionGraphState) -> dict:
    """
    日期检查占位节点，实际路由由条件边处理
    """
    return {}
...
def route_date_type(state: TwinsInspectionGraphState) -> str:
    """
    根据日期类型路由
    巡检态势未来日期直接拒绝
    """
...
def future_date(state: TwinsInspectionGraphState) -> dict:
    """
    未来时间直接提醒，不查询
    """
    return {
        ...
        "events": [
            {
                ...
                "data": {
                    "type": "answer",
                    "content": "巡检数据目前只能查询今天及历史的统计，未来的数据暂时无法预测哦。",
                },
            },
            {"event": "custom", "data": {"type": "done"}},
        ],
...
def vague_date(state: TwinsInspectionGraphState) -> dict:
    """
    模糊时间先让用户确认具体范围
    """
    date_slots = state["slots"].get("date", {})
    ...
    question = (
        f"您想查询近几天的巡检数据？可以直接回复具体天数，例如“近两天”、“近四天”，"
        f"或选择：{options_str}"
    )
...
def make_call_tool_node(tools: dict, llm=None):
    ...
    构造调用 MCP 工具的节点
    """
    async def call_tool(state: TwinsInspectionGraphState) -> dict:
        ...
        try:
            ...
            answer_text = await _llm_format_inspection_result(
...
def finalize(state: TwinsInspectionGraphState) -> dict:
    """
    标记流程完成
    """
    return {
...
# 图构建
# ============================================================
def build_twins_inspection_graph(tools: dict, llm=None) -> StateGraph:
    """
    构建孪生巡检 LangGraph 状态图
    ...
    """
    workflow = StateGraph(TwinsInspectionGraphState)
...
# 辅助：加载 MCP 工具
# ============================================================
async def load_twins_inspection_tools() -> dict:
    """
    从 MCP 配置加载孪生巡检工具，并以工具名为 key 返回
    类级缓存由调用方维护
    """
    ...
    try:
        ...
        logger.error("加载 MCP 孪生巡检工具超时")
        return {}
    except Exception as e:
        logger.error(f"加载 MCP 孪生巡检工具失败: {e}", exc_info=True)
        return {}

    filtered = [t for t in tools if t.name in java_tool_names]
    if not filtered:
        logger.warning(f"未找到任何孪生巡检 MCP 工具，期望 {java_tool_names}")
