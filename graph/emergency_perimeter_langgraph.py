...
# 与 Java 侧 PerimeterMcpServerConfig / PlatformPerimeterMcp 对齐：
#   key_metrics           -> perimeter:getKeyMetrics
#   area_overview         -> perimeter:getAreaOverview
#   perimeter_alarm_stats -> perimeter:getPerimeterAlarmStats
#   alarm_overview        -> perimeter:getAlarmOverview
# ============================================================
_JAVA_TOOL_MAP = {
    "key_metrics": "perimeter:getKeyMetrics",
    "area_overview": "perimeter:getAreaOverview",
...
# 周界子类型反问的固定选项顺序（与 _JAVA_TOOL_MAP / 反问文案顺序一致），
...
# ============================================================
# 告警一览列表优化：紧凑化
# 背景：告警一览一次返回整个 rows 列表，每条还带抓拍链接等长字段，
#       原始 JSON 直接喂 LLM 又慢又费 token。
# 这里在每次调用 MCP 后，把原始 rows 压成紧凑文本
# （只保留关键字段、限制条数），再喂给 LLM。
...
# 告警一览单行保留字段（截断长字段）
_ALARM_KEEP_FIELDS = [
    "alarmName", "areaName", "alarmLevel", "handleStatus",
    "alarmTime", "alarmReason",
]


def _compact_perimeter_alarm_rows(raw_text: str) -> str:
    """
    把周界告警一览 rows 列表的原始 JSON 压成紧凑文本，再喂给 LLM

    原始数据结构（来自平台）：
      { msg; total; code; rows: [...] }

    只保留 _ALARM_KEEP_FIELDS 中列出的关键字段（字段值过长时截断），
    并限制最多 _MAX_ROWS 条。字段名以 Time 结尾且值为纯数字时，
    按毫秒时间戳格式化为 MM-dd HH:mm。解析失败时兜底返回原始文本。
    """
    ...
    for idx, row in enumerate(rows[:_MAX_ROWS], start=1):
        if not isinstance(row, dict):
            ...
            continue
        parts = []
        for field in _ALARM_KEEP_FIELDS:
            value = row.get(field)
            if value is None:
                continue
            value_str = str(value)
            # 毫秒时间戳转可读时间，例如 alarmTime=1725926400000 -> 10-10 08:00
...
async def _llm_format_perimeter_result(
    ...
    try:
        ...
        requirement = {
            "key_metrics": "列出周界在线/离线防区设备数、今日告警数等关键指标；",
            "area_overview": "列出各防区的名称与布防状态（布防/撤防/在线/离线）；",
            "perimeter_alarm_stats": "列出周界告警按区域/时段的统计与小时曲线数据；",
            "alarm_overview": "按时间列出最近的周界告警（类型/等级/防区/时间/处理状态）；",
        ...
        prompt = (
            "你是智慧园区周界态势助手。下面是一次 MCP 工具查询的原始返回，"
        ...
        logger.warning(f"LLM 格式化周界态势结果失败，降级为原样返回: {e}")
...
class EmergencyPerimeterGraphState(TypedDict):
    # 从当前意图状态中提取的参数
...
# 图节点函数
# ============================================================
def init_state(state: EmergencyPerimeterGraphState) -> dict:
    """
...
def check_missing_params(state: EmergencyPerimeterGraphState) -> dict:
    """
    检查缺失参数
    周界后端只支持 4 类查询，query_type 落到 count 兜底
...
def route_missing_params(state: EmergencyPerimeterGraphState) -> str:
    """
    条件路由：根据是否缺失参数决定下一步
    """
...
def ask_param(state: EmergencyPerimeterGraphState) -> dict:
    """
    参数缺失时生成追问问题
    """
    ...
    if "query_type" in missing:
        ...
        # （仿消防态势的 _pending_type_clarify / _type_options 模式）
        slots = state["slots"]
...
def give_up(state: EmergencyPerimeterGraphState) -> dict:
    """
    追问次数超限，放弃当前任务
    """
    return {
...
def check_date(state: EmergencyPerimeterGraphState) -> dict:
    """
    日期检查占位节点，实际路由由条件边处理
    """
    return {}
...
def route_date_type(state: EmergencyPerimeterGraphState) -> str:
    """
    根据日期类型路由
    周界态势未来日期直接拒绝
    """
...
def future_date(state: EmergencyPerimeterGraphState) -> dict:
    """
    未来时间直接提醒，不查询
    """
    return {
        ...
        "events": [
            {
                ...
                "data": {
                    "type": "answer",
                    "content": "周界数据目前只能查询今天及历史的统计，未来的数据暂时无法预测哦。",
                },
            },
            {"event": "custom", "data": {"type": "done"}},
        ],
...
def vague_date(state: EmergencyPerimeterGraphState) -> dict:
    """
    模糊时间先让用户确认具体范围
    """
    date_slots = state["slots"].get("date", {})
    ...
    question = (
        f"您想查询近几天的周界数据？可以直接回复具体天数，例如“近两天”、“近四天”，"
        f"或选择：{options_str}"
    )
...
def make_call_tool_node(tools: dict, llm=None):
    ...
    构造调用 MCP 工具的节点
    """
    async def call_tool(state: EmergencyPerimeterGraphState) -> dict:
        ...
        try:
            ...
            # 告警一览的 rows 列表原始 JSON 很大（含抓拍链接等长字段），
            # 先压成紧凑文本（只保留关键字段、限制条数）再喂 LLM，
            # 大幅减少 token、加快回复
            if query_type == "alarm_overview":
                raw_text = _compact_perimeter_alarm_rows(raw_text)

            answer_text = await _llm_format_perimeter_result(
...
def finalize(state: EmergencyPerimeterGraphState) -> dict:
    """
    标记流程完成
    """
    return {
...
# 图构建
# ============================================================
def build_emergency_perimeter_graph(tools: dict, llm=None) -> StateGraph:
    """
    构建周界态势 LangGraph 状态图
    ...
    """
    workflow = StateGraph(EmergencyPerimeterGraphState)
...
# 辅助：加载 MCP 工具
# ============================================================
async def load_emergency_perimeter_tools() -> dict:
    """
    从 MCP 配置加载周界态势工具，并以工具名为 key 返回
    类级缓存由调用方维护
    """
    ...
    try:
        ...
        logger.error("加载 MCP 周界态势工具超时")
        return {}
    except Exception as e:
        logger.error(f"加载 MCP 周界态势工具失败: {e}", exc_info=True)
        return {}

    filtered = [t for t in tools if t.name in java_tool_names]
    if not filtered:
        logger.warning(f"未找到任何周界态势 MCP 工具，期望 {java_tool_names}")
