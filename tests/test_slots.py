...
class TestPersonSlots:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("看看张三上午的轨迹", "trace"),
            ("今天进入多少人", "enter"),
            # 回归：leave 正则原缺「离.*多少」通配，「昨天离开园区多少人」
            # 被 realtime 的「园区.*多少.*人」兜底接走（2026-09-23 冒烟发现）
            ("昨天离开园区多少人", "leave"),
            ("离园多少人", "leave"),
            # 对照行：enter 一直有「进.*多少」，防后续改动把两边改不对称
            ("昨天进入园区多少人", "enter"),
            ("今天离开多少人", "leave"),
            ("最近人员流动趋势怎么样", "flow"),
            ("园区人员结构分布", "structure"),
...
class TestDeviceQuerySlots:
    ...
    @pytest.mark.parametrize(
        ...
        [
            # 资产库的 syncSource：0门禁 1道闸 2梯控 3监控 4入侵报警 5广播 6水表 7电表
            ("查下监控设备列表", "3"),
            ("园区有哪些摄像头", "3"),
            ("B栋有哪些门禁设备", "0"),
            ("道闸设备清单", "1"),
            ...
            ("梯控设备有哪些", "2"),
            ("入侵报警设备清单", "4"),
            # 别名重叠时取最长的那个："车牌识别闸机"是道闸，不能被"闸机"抢成门禁，
            # "入侵报警"同理不能被"报警"抢走
            ("车牌识别闸机列表", "1"),
            ("有哪些报警设备", "4"),
            # 资产库没有的类型：不落码，按"全部设备"查
            ("看下空调设备列表", ""),
            ("设备列表", ""),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("设备名称是门禁001的详情", "门禁001"),
            ("“A栋枪机”的详情", "A栋枪机"),
            # 资产库的设备编号是多段带分隔符的，必须整串抽出来
            # （只认"字母+数字"会截成 JK-001，拿去搜设备搜不到）
            ("CY-HIK-JK-001-0001的详情", "CY-HIK-JK-001-0001"),
            ("看下设备编号CY-DH-MJ-002-0001的详情", "CY-DH-MJ-002-0001"),
            ("设备列表", ""),
            ("设备详情", ""),
        ],
    )
